...
class SectionsRecyclerViewAdapter(
    ...
    override fun onBindViewHolder(holder: CategoryViewHolder, position: Int) {
        ...
        holder.recyclerViewCategoryBinding.sectionButton.setOnClickListener {
            val layoutId:Int = view.resources.getIdentifier(categories[position].layoutName, "id", view.context.packageName)
            if(layoutId > 0)
                navController.navigate(layoutId)
            else
                Toast.makeText(view.context, "Nie można odnaleźć wybranej sekcji, spróbuj ponownie później.", Toast.LENGTH_SHORT).show()
        }
    }
